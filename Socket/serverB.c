...
int search(int link_id) {
	...
	while(!feof(fp)) {
		...
		if(strtod(token, NULL) == (double)link_id){
			database_b[i++] = (double)link_id;
			token = strtok(NULL, ",");
			database_b[i++] = strtod(token, NULL);
			token = strtok(NULL, ",");
			database_b[i++] = strtod(token, NULL);
			token = strtok(NULL, ",");
			database_b[i++] = strtod(token, NULL);
...
int main(int argc, char const *argv[]) {
	...
	while(1) {  // main accept() loop
		...
		if (match == 1 && sendto(sockfd, &database_b, sizeof database_b, 0, (struct sockaddr *)&their_addr, addr_len) == -1) {
			perror("serverB: sendto");
			exit(1);
		}
